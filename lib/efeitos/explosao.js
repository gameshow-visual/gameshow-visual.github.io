...
const triggers = document.querySelector('[explosao');
const defaults = {
...
function explosao(x = 1) {
	for (let i = 1; i <= x; i++) {
		setTimeout(() => {
			const rect = triggers.getBoundingClientRect();
			const center = {
		...
		}, i * ((Math.floor(Math.random() * 5) + 1) * 100));
	}
}