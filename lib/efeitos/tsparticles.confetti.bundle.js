...
(function (global, factory) {
    ...
    (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory((global.__tsParticlesInternals = global.__tsParticlesInternals || {}, global.__tsParticlesInternals.bundles = global.__tsParticlesInternals.bundles || {}, global.__tsParticlesInternals.bundles.confetti = global.__tsParticlesInternals.bundles.confetti || {})));
    ...
    function setColorAnimation(colorValue, colorAnimation, reduceFactor) {
        ...
        if (colorValue.enable) {
            ...
            colorValue.decay = decayOffset$1 - getRangeValue(colorAnimation.decay);
    ...
    const sides$3 = 12, maxAngle$3 = 360, minAngle = 0;
    class CircleDrawer {
        draw(data) {
            drawCircle(data);
        }
    ...
    class PaintUpdater {
        ...
        init(particle) {
            ...
            if (fill) {
                ...
                particle.fillOpacity = defaultOpacity$1;
            }
            if (stroke) {
                ...
                particle.strokeOpacity = defaultOpacity$1;
    ...
    const firstIndex$1 = 0, index2 = 1, index3 = 2, index4 = 3;
    function drawPath(ctx, radius, path) {
        const firstSegment = path.segments[firstIndex$1];
        if (!firstSegment) {
            return;
        }
        const firstValue = firstSegment.values[firstIndex$1];
        if (!firstValue) {
            return;
        }
        ctx.moveTo(firstValue.x * radius, firstValue.y * radius);
        for (const segment of path.segments) {
            const value = segment.values[firstIndex$1];
            if (!value) {
                continue;
            }
        ...
        for (let i = path.segments.length - lengthOffset; i >= minLength; i--) {
            ...
            const value = segment.values[firstIndex$1], index2 = 1, index3 = 2, segmentValue2 = segment.values[index2], segmentValue3 = segment.values[index3];
            switch (segment.type) {
                case SegmentType.line:
                    if (!value) {
                        break;
    ...
    function initParticle(pluginManager, particle) {
        ...
        particle.roll = {
            ...
            speed: getRangeValue(rollOpt.speed) / maxAngle$2,
        };
    ...
    async function setConfetti(engine, params) {
        ...
        const create = async () => {
            const particlesOptions = convertOptions(actualOptions, params, opacitySpeed), newContainer = await engine.load({
                id: params.id,
    ...
    class MovePluginInstance {
        ...
        particleCreated(particle) {
            ...
            particle.moveDecay = decayOffset$1 - getRangeValue(moveOptions.decay);
                ...
                if (this._shape) {
                    ...
                    if (shapePosData) {
                        ...
                        if (replaceData.color && shapePosData.color) {
                            setParticlesOptionsFillColor(particlesOptions, shapePosData.color, replaceData.opacity ? (shapePosData.opacity ?? defaultOpacity$2) : defaultOpacity$2, true);
                        }
                    }
                    else {
                        position = null;
    ...
    exports.decayOffset = decayOffset$1;
    ...
    exports.defaultOpacity = defaultOpacity$2;
